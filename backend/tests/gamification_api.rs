...
async fn fresh_users_start_at_zero() {
    ...
    let (status, stats) =
        request(&app, "GET", "/api/gamification/stats", Some(&token), None, "10.0.40.1").await;
    assert_eq!(status.as_u16(), 200, "{stats}");
    assert_eq!(stats["xp"], 0);
    assert_eq!(stats["streak_days"], 0);
    assert_eq!(stats["earned_count"], 0);
    assert!(stats["total_count"].as_u64().unwrap() >= 100, "the 100-achievement catalog is seeded");
...
async fn real_activities_earn_xp_and_an_achievement() {
    ...
    let (status, stats) =
        request(&app, "GET", "/api/gamification/stats", Some(&token), None, "10.0.41.3").await;
    assert_eq!(status.as_u16(), 200, "{stats}");
    assert!(stats["xp"].as_i64().unwrap() >= 3, "correction awards XP: {stats}");
    assert_eq!(stats["streak_days"], 1, "activity today starts the streak: {stats}");
    assert!(stats["earned_count"].as_u64().unwrap() >= 1, "first-correction badge earned: {stats}");
...
async fn same_day_activity_does_not_double_the_streak() {
    ...
    let (_, stats) = request(&app, "GET", "/api/gamification/stats", Some(&token), None, "10.0.42.5").await;
    assert_eq!(stats["streak_days"], 1, "one day, one streak: {stats}");
    assert!(stats["xp"].as_i64().unwrap() >= 6, "both corrections paid out: {stats}");
}

#[tokio::test]
async fn stats_requires_authentication() {
    let (status, body) = request(&app(30, 120), "GET", "/api/gamification/stats", None, None, "10.0.43.1").await;
